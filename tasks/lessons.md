...
### 用户纠偏：时间线的 active 高亮必须直观可见，轨道滚动不能和页面滚动抢控制权
- 触发场景：用户反馈“会话滚动高亮没有做好，看不到”，并指出鼠标移到时间线柱子上后滚轮滚动发涩，说明 active dot 的视觉强调不够，同时自定义 wheel 处理和原生滚动存在互相争抢。
- 本次修正规则：
  - active marker 不仅要算对，还要在视觉上明显区分；高亮样式必须在超长会话、滚动窗口和窄轨道场景下仍然清晰。
  - 时间线轨道若自身可滚动，优先使用原生滚动和 `overscroll` 约束，避免内容脚本再手动接管 wheel 造成卡顿或双重滚动。
  - 长会话时间线要同时控制“可见点数”和“轨道宽度”，不能为了滚动窗口把柱子做得过宽或压迫正文。

### 用户纠偏：Popup 不能走“精简 popup + 单独完整设置页”的分裂架构
- 触发场景：用户明确要求“完整设置不要单独设置了，就挪到 popup 里面”，并指出当前 popup 太花、太人工设计感，说明上次把完整设置拆到独立页面违背了当前产品预期。
- 本次修正规则：
  - action popup 应直接承载完整设置，不能把核心设置再分流到单独的“完整设置”入口。
  - 设计上优先简约、克制和高信息密度，不要用过强装饰、过多渐变和过度包装的 hero 结构。
  - 改 popup 结构时，先核对 `popup.js` 已绑定的全部控件 id，确保完整设置项没有因为 UI 重排而遗漏。

### 用户纠偏：文件夹最常用的“创建并分配到当前会话”不能依赖费劲的表单/下拉流程
- 触发场景：用户反馈“文件夹里的 UI 和交互应该再优化一下，交互总是感觉很费劲”，说明当前会话分配主要依赖 `select`，而创建后也不会自动挂到当前会话，常用路径过长。
- 本次修正规则：
  - 当前会话的文件夹分配优先用点击式快捷操作，而不是把 `select` 作为主交互。
  - 从文件夹面板或侧栏创建新文件夹时，如果当前正在聊天会话中，新文件夹应默认直接分配给当前会话。
  - 文件夹 UI 要优先优化“高频动作”而不是低频管理动作：先让分配、取消分配、快速新建顺手，再考虑改名/删除。
